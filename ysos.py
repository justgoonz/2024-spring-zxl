...
# action='store_true'用户在命令行中指定了该参数，则该参数的值将被存储为 True，否则将被存储为 False。
# verbose：输出更多调试信息
parser.add_argument('-v', '--verbose', action='store_true',
                    help='Enable verbose output')
# 模拟执行，查看操作可能产生的影响，而不实际执行。（试运行）                    
...
# shell - whether use shell
    prog = subprocess.Popen(cmd, cwd=workdir, shell=shell)
# wait until subprocess finished
    prog.wait()
    ...
    for app in apps:
        ...
        with open(os.path.join(app_path, 'Cargo.toml'), 'r') as f:
            for line in f.readlines():
                if 'name' in line: # find the line contain app's name
                    app_name = line.split('"')[1]
                    break

        info('Building', f'app {app}...')
        execute_command([cargo_exe, 'build', profile], app_path)
        compile_output = os.path.join(os.getcwd(), 'target', 'x86_64-unknown-ysos', profile_dir, app_name)
